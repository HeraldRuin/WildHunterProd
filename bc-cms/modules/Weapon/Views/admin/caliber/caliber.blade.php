...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{!empty($recovery) ? __('Recovery') : __("All Calibers")}}</h1>
            <div class="title-actions">
                @if(empty($recovery))
                    <a href="{{route('caliber.admin.create')}}" class="btn btn-primary">{{__("Add new caliber")}}</a>
                @endif
            </div>
        </div>
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if(!empty($rows))
                    <form method="post" action="{{route('caliber.admin.bulkEdit')}}" class="filter-form filter-form-left d-flex justify-content-start">
                        {{csrf_field()}}
                ...
                <form action="" class="bc-form-item">
                    <div class="table-responsive">
                        <table class="table table-hover">
                                ...
                                @foreach($rows as $row)
                                    <tr class="{{$row->status}}">
                                        ...
                                        <td class="title">
                                            ...
                                            @endif
                                            <a href="{{route('caliber.admin.edit',['id'=>$row->id])}}">{{$row->title}}</a>
                                        </td>
                                        ...
                                        <td>
                                            @if(empty($recovery))
                                                <a href="{{route('caliber.admin.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Edit')}}</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                            @else
                                <tr>
                                    <td colspan="7">{{__("No calibers found")}}</td>
                                </tr>
                            @endif
                            </tbody>
                        </table>
