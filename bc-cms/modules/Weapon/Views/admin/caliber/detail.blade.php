...
@section('content')
    <form action="{{ route('caliber.admin.store', ['id' => $row->id ? $row->id : '-1', 'lang' => request()->query('lang')]) }}"
        method="post">
        @csrf
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{ $row->id ? __('Edit: ') . $row->name : __('Add new caliber') }}</h1>
                    @if ($row->slug)
                        <p class="item-url-demo">{{ __('Permalink') }}: {{ url('caliber') }}/<a href="#"
                ...
                <div class="">
                    @if ($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{ $row->getDetailUrl(request()->query('lang')) }}"
                            target="_blank">{{ __('View Weapon') }}</a>
                    @endif
                </div>
            </div>
            ...
            <div class="lang-content-box">
                <div class="row">
                    <div class="col-md-9">
                        @include('Weapon::admin.caliber.content')
