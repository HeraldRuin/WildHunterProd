                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                                                ...
                                                @if(!in_array($booking->status, [\Modules\Booking\Models\Booking::CANCELLED, \Modules\Booking\Models\Booking::COMPLETED]))
                                                    <a class="dropdown-item" href="#" data-toggle="modal" data-target="#cancelBookingModal{{$booking->id}}">{{__('Cancel')}}</a>
                                                @endif
                                            </div>
                                        </div>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('No, keep booking')}}</button>
                                        <button type="button" class="btn btn-danger btn-cancel-booking-confirm" data-id="{{ $booking->id }}">{{__('Yes, cancel')}}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('js')
    <script>
        ...
        $(document).on('click','.btn-cancel-booking-confirm',function (e) {
            e.preventDefault();
            var btn = $(this);
            var id = btn.data('id');
            ...
            btn.prop('disabled', true).addClass('disabled').html(
                ...
                '<span> ' + (btn.text().trim() || '{{__("Processing...")}}') + '</span>'
            );
            
            $.ajax({
                url:bookingCore.url+'/booking/'+id+'/cancel',
                method:"post",
                data:{
                    _token: '{{csrf_token()}}'
                },
                ...
                success:function (res) {
                    if(res.status){
                        $('#cancelBookingModal'+id).modal('hide');
                        alert(res.message);
                        window.location.reload();
                    }else{
                        ...
                        if (btn.data('originalHtml')) {
                            ...
                            btn.text('{{__("Yes, cancel")}}');
                        }
                        alert(res.message || '{{__("Error occurred")}}');
                    }
                },
                error:function (e) {
                    console.log(e);
